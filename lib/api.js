...
export async function getHomePageContent() {
  const gallery = await getClient().fetch(`*[_type == "gallery"][0]{images}`)
  const about = await getClient().fetch(`*[_type == "about"][0]`)
  const hero = await getClient().fetch(
    `*[_type == "banner"][0]{_id, image {asset}, title, subtitle, cta {url, title}}`
  )
  const testimonials = await getClient().fetch(
    `*[_type == "testimonials"]{headline, name, location, testimonial}`
  )
